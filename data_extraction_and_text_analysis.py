...
df = pd.read_excel('E:/blackcoffer/Input.xlsx')
...
for index, row in df.iterrows():
  ...
  file_name = 'E:/blackcoffer/title/' + str(url_id) + '.txt'
...
# Directories
text_dir = "E:/blackcoffer/title"
stopwords_dir = "E:/blackcoffer/StopWords"
sentment_dir = "E:/blackcoffer/MasterDictionary"
...
def main():
    ...
    if os.path.exists(text_dir):
        with open(os.path.join(text_dir,file), 'r', encoding = "utf-8") as file:
            text = file.read()
...
for file in os.listdir(text_dir):
  #x, y = average_words_per_sentence[file]
  avg_per_sent.append(x)
  average_words_per_sentence.append(y)
...
output_df = pd.read_excel('E:/blackcoffer/Output Data Structure.xlsx')

# URL_ID 36, 49 does not exists i,e. page does not exist, throughs 404 error
# so we are going to drop these rows from the table
# output_df.drop([blackassign0036, blackassign0049], axis = 0, inplace=True)
...
for i, var in enumerate(variables):
  output_df.iloc[2:100,i+1] = var

#now save the dataframe to the disk
output_df.to_csv('E:/blackcoffer/Output_Data.csv')
